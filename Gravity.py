...
class point:
	...
	def __eq__(self, o):
		...
		if o == None:
			return False
		return int(self.x) == int(o.x) and int(self.y) == int(o.y) and int(self.z) == int(o.z)

	def distancia_sol(self):
		x_ = math.pow(self.x,2)
		y_ = math.pow(self.y,2)
		z_ = math.pow(self.z,2)
		return math.sqrt(x_+y_+z_)
	...
	for i in range(1,number_of_steps):
		...
		if i % report_freq == 0:
			for index, body_location in enumerate(body_locations_hist):
				...
				x = bodies[index].location.x
				y = bodies[index].location.y
				z = bodies[index].location.z
				nombre = bodies[index].name
				if bodies[index].mayor == None:
					bodies[index].mayor = bodies[index].location.distancia_sol()
					bodies[index].menor = bodies[index].mayor
				else:
					if bodies[index].mayor < bodies[index].location.distancia_sol():
						bodies[index].mayor = bodies[index].location.distancia_sol()
					if bodies[index].menor > bodies[index].location.distancia_sol():
						bodies[index].menor = bodies[index].location.distancia_sol()
...
if __name__ == "__main__":
	...
	motions = run_simulation(bodies, time_step = 100, number_of_steps = 800000, report_freq = 1000)
	
	for current_body in bodies:
		print(f"{current_body.name}-> mayor {current_body.mayor}")
		print(f"\t menor {current_body.menor}")
